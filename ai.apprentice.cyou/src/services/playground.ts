    ...
    try {
      const result = await new Promise<Record<string, unknown>>((resolve, reject) => {
        const stream = cloudinary.uploader.upload_stream(
          { folder: "playground", public_id: publicId, resource_type: "auto" },
          (err, res) => (err || !res ? reject(err || new Error("upload")) : resolve(res as Record<string, unknown>))
        );
  ...
  if (prov === "cloudinary") {
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        cloudinary.uploader.destroy(opts.externalId, { invalidate: true }, (err) => (err ? reject(err) : resolve()));
      });
      return { ok: true };
    } catch (e) {
